...
## Merging
For the most part, student developers should always merge into the branch `dev`. Any merges into `main` will only be from `dev`, and will be done after a confirmation from Dr. van Heuveln. As a student developer, it is important that you resolve any conflicts in `dev` before requesting to merge into main.
